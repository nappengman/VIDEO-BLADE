...
def simple_pooling(x, sample_gap=sample_gap):
    ...
    return x
def padding_with_mean(x,sample_gap=sample_gap):
    x_temp=x[:,:,:-226].reshape(x.size(0),x.size(1),-1,sample_gap,x.size(-1))
    x_mean=x_temp.mean(dim=-2)
    x=torch.concat([x,x_mean],dim=-2)
    return x
def padding_with_sum(x,sample_gap=sample_gap):
    x_temp=x[:,:,:-226].reshape(x.size(0),x.size(1),-1,sample_gap,x.size(-1))
    x_mean=x_temp.sum(dim=-2)
    x=torch.concat([x,x_mean],dim=-2)
    return x
...
#     sparsity=attn[:,:,:int(k.size(-2)*0.2)].sum(dim=-1)
#     return sparsity  
...
def transfer_attn_to_mask(attn, mode="energy", init_k=None, max_retain_ratio=0.7, min_retain_ratio=0.1, energy_threshold=0.95):
    ...
    if mode == "topk":
        ...
        seq = attn.shape[2]  # int
        # 修复警告：使用 .clone().detach() 而不是 torch.tensor()
        min_retain = torch.clamp((seq * min_retain_ratio).to(torch.int), min=1).unsqueeze(-1)
        max_retain = torch.clamp((seq * max_retain_ratio).to(torch.int), min=1).unsqueeze(-1)
        sorted_attn, indices = torch.sort(attn, dim=-1, descending=True)  # (b, h, seq)
        cum_energy = torch.cumsum(sorted_attn, dim=-1)  # (b, h, seq)
        total_energy = cum_energy[..., -1:]  # (b, h, 1)
        energy_mask = cum_energy >= energy_threshold * total_energy  # (b, h, seq)
        k_indices = torch.argmax(energy_mask.int(), dim=-1)  # (b, h)
        unsatisfied = (cum_energy[..., -1] < energy_threshold * total_energy.squeeze(-1))  # (b, h)
        k_indices = torch.where(unsatisfied, torch.full_like(k_indices, seq), k_indices)  # (b, h)
        k_indices = torch.minimum(torch.maximum(k_indices, min_retain), max_retain)  # (b, h)
        pos_indices = torch.arange(seq, device=device).view(1, 1, seq)  # (1, 1, seq)
        k_indices_exp = k_indices.unsqueeze(-1)  # (b, h, 1)
        keep_mask = pos_indices < k_indices_exp  # (b, h, seq)
...
def adaptive_block_sparse_attn(q, k, v):
    ...
    with torch.no_grad():
        ...
        mask = transfer_attn_to_mask(
            ...
            mode="energy",
            init_k=None,
            max_retain_ratio=max_retain_ratio_,
            min_retain_ratio=min_retain_ratio_,
            energy_threshold=0.95
        )
    # k=padding_with_mean(k,sample_gap=sample_gap)
    # v=padding_with_mean(v,sample_gap=sample_gap)
    # L= k.size(-2)//block_size+1
    # mask_temp=torch.ones(mask.shape[0],mask.shape[1],mask.shape[2],L,device=q.device,dtype=torch.bool)
    # mask_temp[:,:,:,:mask.shape[-1]]=mask
    # mask = mask_temp
...
class AdaptiveBlockSparseAttnTrain(nn.Module):
    ...
    def forward(self, q, k, v):
        ...
        # Print average sparsity every 800 calls
        if self.sparsity_counter % 800 == 0:

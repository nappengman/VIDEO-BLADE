...
def standard_attn(q,k,v,block_size=BLOCK_SIZE):
    mask=torch.ones([q.size(0),q.size(1),upround(q.size(2)/block_size),upround(k.size(2)/block_size)],device=q.device,dtype=torch.bool)
...
def pad_to_multiple(x, multiple):
    ...
    return x
def random_sample_tokens(x, block_size=BLOCK_SIZE, sample_num=NUM_KEEP):
    """
    ...
    return sampled
# @timeit
def efficient_attn_with_pooling(q, k, v, block_size=BLOCK_SIZE, num_keep=NUM_KEEP):
    """
    计算下采样后的注意力池化：
...
def simple_pooling(x, sample_gap=SAMPLE_GAP):
    ...
    return x
# @timeit
def org_attn_with_pooling(q, k, v, block_size=BLOCK_SIZE):
...
class GilbertRearranger(nn.Module):
    ...
    def _is_same(self, datainfo):
        if self.initialized:
            if self.width == datainfo["width"] and \
               self.height == datainfo["height"] and \
               self.depth == datainfo["depth"] and \
               self.text_length == datainfo["text_length"]:
                   return True
        return False

    def _update_seq_info(self, datainfo, device):
        if self._is_same(datainfo):
            return
        self.initialized = True

        self.width = datainfo["width"]
        self.height = datainfo["height"]
        self.depth = datainfo["depth"]
        self.text_length = datainfo["text_length"]

        self.total_elements = self.width * self.height * self.depth

        coord_to_index = self._gilbert3d_with_index(self.width, self.height, self.depth)
        ...
        self.original_order2gilbert_order = torch.tensor(original_order2gilbert_order, dtype=torch.long).to(device)
        self.gilbert_order2original_order = torch.tensor(gilbert_order2original_order, dtype=torch.long).to(device)
        
        print(f"Update GilbertRearranger Seq-Info: {self.width}, {self.height}, {self.depth}, {self.text_length}")
...
# @timeit
def transfer_attn_to_mask(attn, mode="energy", init_k=None, max_retain_ratio=MAX_RETAIN_RATIO, min_retain_ratio=MIN_RETAIN_RATIO, energy_threshold=ENERGY_THRESHOLD):
    """
    将注意力权重转换为掩码矩阵。
...
def adaptive_block_sparse_attn(q, k, v, block_size=BLOCK_SIZE, sample_gap=SAMPLE_GAP, max_retain_ratio=MAX_RETAIN_RATIO, min_retain_ratio=MIN_RETAIN_RATIO):
    """
...
class AdaptiveBlockSparseAttnTrain(nn.Module):
    ...
    # @timeit
    def forward(self, q, k, v, datainfo={'width': 0, 'height': 0, 'depth': 0, 'text_length': 0}):
        assert self.gilbert_rearranger.initialized, "GilbertRearranger 未初始化"
        # self.gilbert_rearranger._update_seq_info(datainfo, q.device)
